...
exports.StackExchangeScraper = class StackExchangeScraper extends BaseScraper
  ...
  _check_args : (args) ->
    if not(args.account_id?)
      new Error "Bad args to Stack Exchange proof: no account_id given"
    else if not(args.user_id?)
      new Error "Bad args to Stack Exchange proof: no user_id given"
    else if not(args.site_host?)
      new Error "Bad args to Stack Exchange proof: no site_host given"
    else if not (args.name?) or (args.name isnt 'stackexchange')
      new Error "Bad args to Stack Exchange proof: type is #{args.name}"
    else
      null

  # ---------------------------------------------------------------------------

  hunt2 : ({site_host, user_id, account_id, proof_text_check, name}, cb) ->
    ...
    key = "LZaXyoVe1OF7JOsWxGzYjg(("
    filter = "t*er2anz(AGnsZNE9rO474S586)NSFu0acU"
    url = "https://api.stackexchange.com/2.2/users/{user_id}?site={site_host}&filter=#{filter}&key=#{key}"
